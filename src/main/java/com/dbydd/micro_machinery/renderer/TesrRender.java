...
public class TesrRender extends TileEntitySpecialRenderer<TESRTestTile> {//在这个<>里填上要绑定的tileentity，然后去RegisteryHandler绑定


    @Override
    public void render(TESRTestTile te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        GlStateManager.pushAttrib();
        GlStateManager.pushMatrix();

        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder buffer = tessellator.getBuffer();//获得buffer
