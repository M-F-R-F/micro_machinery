...
public class LatheScreen extends ScreenBase<LatheContainer> {
    ...
    public LatheScreen(LatheContainer screenContainer, PlayerInventory inv, ITextComponent titleIn) {
        super(screenContainer, inv, titleIn, new ResourceLocation(MicroMachinery.MODID, "textures/gui/lathe.png"), 176, 182);
    }
    ...
    protected void init() {
        ...
        addButton(new ButtonBase(this.leftPos + 65, this.topPos + 47, 14, 14, "", "button.action.turning", 214, 126, 214, 112, button -> {
        ...
        addButton(new ButtonBase(this.leftPos + 81, this.topPos + 47, 14, 14, "", "button.action.grinding", 228, 126, 228, 112, button -> {
            CompoundTag compoundNBT = new CompoundTag();
            compoundNBT.putString("action", TileLathe.Action.GRINDING.name());
            TileClientToServerSyncChannel.INSTANCE.sendToServer(new TileClientToServerSyncPackage(compoundNBT, container.getLathe().getPos()));
        }, this));

        addButton(new ButtonBase(this.leftPos + 97, this.topPos + 47, 14, 14, "", "button.action.planing", 242, 126, 242, 112, button -> {
            CompoundTag compoundNBT = new CompoundTag();
            compoundNBT.putString("action", TileLathe.Action.PLANING.name());
            TileClientToServerSyncChannel.INSTANCE.sendToServer(new TileClientToServerSyncPackage(compoundNBT, container.getLathe().getPos()));
        }, this));

        addButton(new ButtonBase(this.leftPos + 65, this.topPos + 63, 14, 14, "", "button.action.boring", 172, 126, 172, 112, button -> {
            CompoundTag compoundNBT = new CompoundTag();
            compoundNBT.putString("action", TileLathe.Action.BORING.name());
            TileClientToServerSyncChannel.INSTANCE.sendToServer(new TileClientToServerSyncPackage(compoundNBT, container.getLathe().getPos()));
        }, this));

        addButton(new ButtonBase(this.leftPos + 81, this.topPos + 63, 14, 14, "", "button.action.drilling", 186, 126, 186, 112, button -> {
            CompoundTag compoundNBT = new CompoundTag();
            compoundNBT.putString("action", TileLathe.Action.DRILLING.name());
            TileClientToServerSyncChannel.INSTANCE.sendToServer(new TileClientToServerSyncPackage(compoundNBT, container.getLathe().getPos()));
        }, this));

        addButton(new ButtonBase(this.leftPos + 97, this.topPos + 63, 14, 14, "", "button.action.milling", 200, 126, 200, 112, button -> {
            CompoundTag compoundNBT = new CompoundTag();
