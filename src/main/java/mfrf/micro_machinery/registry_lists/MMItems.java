...
public class MMItems {
        ...
        public static final RegistryObject<Item> ICON = ITEM_REGISTER.register("micro_machinery_logo", MMItemBase::new),
                        // hammer
                        STONE_HAMMER = ITEM_REGISTER.register("stone_hammer",()->new MMHammerBase(4.0f, -3.1f, ItemTier.STONE,
                                        new Item.Properties().m_41491_(MicroMachinery.MMTAB).addToolType(
                                                        ToolType.PICKAXE,
                                                        ItemTier.STONE.getHarvestLevel()),
                                        )),
                        BRONZE_HAMMER = new MMHammerBase(5.0f, -3.15f, ItemTier.IRON,
                                        new Item.Properties().m_41491_(MicroMachinery.MMTAB).addToolType(
                                                        ToolType.PICKAXE,
                                                        ItemTier.IRON.getHarvestLevel()),
                                        "bronze_hammer"),
                        IRON_HAMMER = new MMHammerBase(6.0f, -3.2f, ItemTier.DIAMOND,
                                        new Item.Properties().m_41491_(MicroMachinery.MMTAB).addToolType(
                                                        ToolType.PICKAXE,
                                                        ItemTier.DIAMOND.getHarvestLevel()),
                                        "iron_hammer"),
                        SUPERHAMMER = new MMHammerBase(8.0f, 0.0f, EnumToolTier.HSS,
                                        new Item.Properties().m_41491_(MicroMachinery.MMTAB).addToolType(
                                                        ToolType.PICKAXE,
                                                        EnumToolTier.HSS.getHarvestLevel()),
                                        "superhammer"),
                        // //axe
                        // BRONZE_AXE = new MMAxeBase(EnumToolTier.BRONZE, 1, -3.1f, new
                        // Item.Properties().group(ItemGroup.TOOLS).addToolType(ToolType.AXE,
                        // EnumToolTier.BRONZE.getHarvestLevel()), "bronze_axe"),
                        // TUNGSTEN_STEEL_AXE = new MMAxeBase(ItemTier.DIAMOND, 2, -3.1f, new
                        // Item.Properties().group(ItemGroup.TOOLS).addToolType(ToolType.AXE,
                        // ItemTier.DIAMOND.getHarvestLevel()), "tungsten_steel_axe"),
                        // HSS_AXE = new MMAxeBase(EnumToolTier.HSS, 4, -3.0f, new
                        // Item.Properties().group(ItemGroup.TOOLS).addToolType(ToolType.AXE,
                        // EnumToolTier.HSS.getHarvestLevel()), "hss_axe"),
                        // //sword
                        // BRONZE_SWORD = new MMSwordBase(EnumToolTier.BRONZE, 2, -2.4f, COMBAT,
                        // "bronze_sword"),
                        // TUNGSTEN_STEEL_SWORD = new MMSwordBase(ItemTier.DIAMOND, 4, -2.6f, COMBAT,
                        // "tungsten_steel_sword"),
                        // HSS_SWORD = new MMSwordBase(EnumToolTier.HSS, 7, -2.2f, COMBAT, "hss_sword"),
                        // //food
                        // GOLDEN_APPLE_DROPS = new MMItemBase(FOOD, "golden_apple_drops", new
                        // Food.Builder().effect(() -> new EffectInstance(Effects.REGENERATION, 200, 1),
                        // 1.0F).effect(() -> new EffectInstance(Effects.RESISTANCE, 3000, 0),
                        // 1.0F).effect(() -> new EffectInstance(Effects.FIRE_RESISTANCE, 3000, 0),
                        // 1.0F).effect(() -> new EffectInstance(Effects.ABSORPTION, 1200, 3),
                        // 1.0F).hunger(3).setAlwaysEdible().saturation(1).build()),
                        // GOLDEN_APPLE_JAM_BUN = new MMItemBase(FOOD, "golden_apple_jam_bun", new
                        // Food.Builder().effect(() -> new EffectInstance(Effects.REGENERATION, 200, 1),
                        // 1.0F).effect(() -> new EffectInstance(Effects.RESISTANCE, 3000, 0),
                        // 1.0F).effect(() -> new EffectInstance(Effects.FIRE_RESISTANCE, 3000, 0),
                        // 1.0F).effect(() -> new EffectInstance(Effects.ABSORPTION, 1200, 3),
                        // 1.0F).hunger(10).saturation(1).build()),
                        // APPLE_DROPS = new MMItemBase(FOOD, "apple_drops", new
                        // Food.Builder().effect(() -> new EffectInstance(Effects.REGENERATION, 50, 1),
                        // 1.0F).effect(() -> new EffectInstance(Effects.ABSORPTION, 1200, 0),
                        // 1.0F).hunger(2).setAlwaysEdible().saturation(1).build()),
                        // APPLE_JAM_BUN = new MMItemBase(FOOD, "apple_jam_bun", new
                        // Food.Builder().effect(() -> new EffectInstance(Effects.REGENERATION, 50, 1),
                        // 1.0F).effect(() -> new EffectInstance(Effects.ABSORPTION, 1200, 0),
                        // 1.0F).hunger(8).saturation(1).build()),
