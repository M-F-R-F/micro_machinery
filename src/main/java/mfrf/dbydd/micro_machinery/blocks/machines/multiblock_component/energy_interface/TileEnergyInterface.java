...
public class TileEnergyInterface extends TilePlaceHolder implements IEnergyStorage, ITickableTileEntity {
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
        if (cap == CapabilityEnergy.ENERGY && side == world.getBlockState(pos).get(BlockEnergyInterface.FACING)) {
    ...
    private LazyOptional<IEnergyStorage> getFacingEnergyCapability() {
        TileEntity tileEntity = world.getTileEntity(pos.offset(world.getBlockState(pos).get(BlockEnergyInterface.FACING)));
        if (tileEntity != null) {
