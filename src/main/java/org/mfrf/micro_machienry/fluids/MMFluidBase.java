package org.mfrf.micro_machienry.fluids;
...
public class MMFluidBase {
    ...
    public MMFluidBase(String name, Block.Properties fluid_block_properties, Function<FluidAttributes.Builder, FluidAttributes.Builder> factory, int tickRate) {
        this.name = name;
        this.fluid_resource_location = new ResourceLocation(MicroMachinery.NAME, "fluids/" + name + "_still");
        this.fluid_flow_resource_location = new ResourceLocation(MicroMachinery.NAME, "fluids/" + name + "_flow");
        ...
        this.fluid_bucket = ITEM_REGISTER.register(name + "_bucket", () -> new BucketItem(this.fluid, MMItemBase.DEFAULT_PROPERTIES.containerItem(Items.BUCKET).maxStackSize(1).group(ItemGroup.MISC)));
